...
2. **造語・略語・カタカナ語の精度** — 「HMR」「型タグ」「コミュ」「placement / placeholder」等。技術用語は正式名で書く。独自造語禁止。曖昧な語は最初に定義する（複数回再発）。Stop hook (`.claude/hooks/stop.py`) が出力後に WARN で検出する
...
6. **短い・曖昧な指示は `/echo-back-intent` で 1 行復唱してから動く** — 「これ」「それ」「もう少し」「枠」「色」など解釈が複数取れる語を含む指示は、自分の解釈を 1 文に直して確認してから着手する。1 セッションで 6 件連発した最頻パターン
7. **不可逆操作の完了は必ず明示報告する** — `git push` / PR 作成・更新 / Issue 書込み等を実行したら、次のメッセージで「○○済み」と 1 行報告する。黙ってターンを終えると「やったなら言って」と指摘される。PostToolUse hook で WARN リマインダが出る
...
- **短い・曖昧な指示を受けたら解釈を 1 行で復唱する** — `/echo-back-intent` を発火させ、解釈を 1 文に直して確認してから着手する。「これ」「それ」「もう少し」「枠」「色」など対象が一意に決まらない指示で必須
